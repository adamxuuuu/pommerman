...
import static utils.Types.DEFAULT_BOMB_AMMO;
import static utils.Types.DEFAULT_BOMB_BLAST;
...
public class Opponent extends GameObject {
    ...
    Opponent(Types.TILETYPE[][] board, Vector2d myPosition, ArrayList<Bomb> bombs,
    ...
    private void init(){
        ...
        recentlyVisitedPositions = new ArrayList<>();
        recentlyVisitedLength = 5;
    }
    ...
    Types.ACTIONS getAction(){
        ...
        if(!unsafeDirections.isEmpty()){
            ...
            if(!directions.isEmpty()) {
                return directionToAction(directions.get(rnd.nextInt(directions.size())));
            }
            else {
        ...
        for (GameObject en: enemies){
            // <Vector2d, Integer>
            for (Map.Entry<Vector2d, Integer> entry : dist.entrySet()) {
        ...
        if (validDirections.size() > 0){
            int actionIdx = rnd.nextInt(validDirections.size());
        ...
        while(!Q.isEmpty()){
            ...
            if(positionIsPassable(board, position, enemies)){
                ...
                for (Types.DIRECTIONS directionToBeChecked : directionsToBeChecked) {
                    ...
                    else if(val == dist_val && rnd.nextFloat() < 0.5){
                        dist.put(new_position, val);
                        prev.put(new_position, position);
                    }
                }
